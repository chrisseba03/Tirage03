...
# Initialisation de l'état pour le test à blanc dans la session
if "test_blanc_gagnant" not in st.session_state:
  st.session_state["test_blanc_gagnant"] = None
...
if mode == "Spectateur / Participant":
  st.info(
      "💡 **Info :** Cette page se met à jour régulièrement pour intégrer les"
      " nouveaux participants au fur et à mesure des validations !"
  )

  st.markdown("---")
  st.markdown(
      "<h3 style='text-align: center;'>⏳ Sablier du Tirage & En Direct</h3>",
  ...
  # Affichage des listes sur 3 colonnes pour les participants validés
  ...
  if data["participants_acceptes"]:
    participants_tries = sorted(
        data["participants_acceptes"], key=lambda x: x.lower()
    )
  ...
  if data["participants_refuses"]:
    refuses_tries = sorted(
        data["participants_refuses"], key=lambda x: x["nom"].lower()
    )
...
# ---------------------------------------------------------
  ...
  tab1, tab2, tab3 = st.tabs(
      ["📝 Ajouter des participants", "🎲 Tirage & Tests", "🗑️ Gestion & Reset"]
  )

  with tab1:
    st.subheader("➕ Ajouter des participants")
    texte_noms = st.text_area(
        "Collez les noms (un par ligne) :",
        height=150,
        placeholder="Nom 1\nNom 2...",
    )
  ...
  with tab2:
    ...
    if st.session_state["test_blanc_gagnant"]:
      st.info(
          f"🧪 **Dernier test à blanc effectué :** Le gagnant simulé était"
          f" **{st.session_state['test_blanc_gagnant']}**."
      )
        ...
        if data["participants_acceptes"]:
          ...
          for _ in range(12):
            ...
            placeholder.markdown(
                f"<h3 style='text-align: center; color: #3b82f6;'>🌀 Boule en"
                f" cours : {temp_winner}</h3>",
                unsafe_allow_html=True,
            )
            time.sleep(0.2)

          gagnant_test = random.choice(data["participants_acceptes"])
          st.session_state["test_blanc_gagnant"] = gagnant_test
          placeholder.empty()
          st.success(
              f"🧪 **[TEST À BLANC] TADAM ! Le gagnant simulé est :"
              f" {gagnant_test}** 🥳"
          )
          st.balloons()
        else:
          st.warning(
              "Ajoutez d'abord des participants pour faire tourner les boules !"
          )
    with col_t2:
      if st.session_state["test_blanc_gagnant"]:
        if st.button("🔄 Réinitialiser le test à blanc"):
          st.session_state["test_blanc_gagnant"] = None
          st.success("Test à blanc réinitialisé !")
          st.rerun()
    ...
    if data["participants_acceptes"]:
      ...
      if data["etat_tirage"] != "Termine":
        if st.button("🎲 LANCER LE VRAI TIRAGE MAINTENANT !"):
          ...
          with st.spinner("Suspense... Les boules tournent dans le boulier ! 🪄"):
            ...
            for _ in range(15):
              temp_winner = random.choice(data["participants_acceptes"])
              placeholder.markdown(
                  f"<h3 style='text-align: center; color: #f59e0b;'>🎰"
                  f" Tirage... {temp_winner}</h3>",
                  unsafe_allow_html=True,
              )
              time.sleep(0.25)
            placeholder.empty()
        ...
        st.success(
            "🏆 Le vrai tirage a déjà eu lieu. Gagnant officiel :"
            f" **{data['gagnant']}**"
        )
    else:
    ...
    st.markdown("---")
    if data["etat_tirage"] == "Termine" or data["gagnant"] is not None:
      if st.button("🔄 Réinitialiser le vrai tirage officiel"):
        data["etat_tirage"] = "En attente"
        data["gagnant"] = None
        save_data(data)
        st.success("Tirage officiel réinitialisé ! Le gagnant a été effacé.")
        st.rerun()
  ...
  with tab3:
    ...
    with st.expander("Gérer / Supprimer des participants validés"):
      if data["participants_acceptes"]:
        admin_participants_tries = sorted(
            data["participants_acceptes"], key=lambda x: x.lower()
        )
